...
edges = [(0, 7), (0, 8), (0, 9), (1, 2), (1, 3), (1, 4), (2, 4), (2, 5), (3, 4),
         (3, 5), (3, 6), (3, 9), (4, 5), (5, 6), (5, 8), (6, 7), (6, 9), (7, 8)]

# representing the colour of vertices
# first index: number of vertex
# second index: colour
colors = [[Bool("c_%s_%s" % (i, j)) for j in range(4)] for i in
...
for n in range(number_of_vertices):
    # only_one_color = Or([And([colors[n][i] if i == j else Not(colors[n][i])
...
# only_one_color = Or([And([Not(colors[0][i]) if i == j else colors[0][i] for
# i in range(4)]) for j in range(4)])
# inner_clause = And([Not(colors[0][i]) if i == 1 else colors[0][i] for i in
# range(4)])
