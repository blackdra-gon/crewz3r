...
table_element_widths: dict[str, int] = {
    ...
    "card_value": 1,
    "trick_start_marker": len(table_elements["trick_start_marker"])
    if table_formatting_rules["mark_trick_start"]
    else 0,
    "task_completion_marker": len(table_elements["task_completion_marker"])
    if table_formatting_rules["mark_task_completion"]
    else 0,
}
...
table_content_widths: dict[str, int] = {
    ...
    + 1
    + table_element_widths["colour_name"]
    + 1
    + table_element_widths["card_value"]
    + 1
    + (
    ...
    "task_completion": len(table_elements["task_completion_marker"]),
}
...
def print_game_parameters(parameters: CrewGameParameters) -> None:
    ...
    p += (
        f"trump cards with maximum value of {parameters.max_trump_value}."
        if parameters.max_trump_value
        else "no trump cards."
    )
    print(p)
    ...
    if ordered_tasks:
        ...
        if ordered_tasks[0].relative_constraint:
            ...
            for i, o_task in enumerate(ordered_tasks[:-1]):
                print(
                    "Task order constraint (relative): "
...
def print_table(
    ...
    for line in lines:
        print(
            column_separator.join(
                [f"{element:^{column_widths[i]}}" for i, element in enumerate(line)]
            )
        )
    print()
...
def print_solution(solution: CrewGameSolution) -> None:
    ...
    for j, trick in enumerate(solution.tricks):
        ...
        t = (
            table_elements["task_completion_marker"]
            if task_completed
